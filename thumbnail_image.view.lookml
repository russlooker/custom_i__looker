- view: thumbnail_image
  ...
  - dimension: age_tier
    type: tier
    tiers: [0, 5, 10, 15, 30, 60, 120, 720, 1440, 2880]
    sql: ${age}
    style: integer

  - dimension: content_type
    sql_case:
      Look: ${TABLE}.look_id is not null
      Dashboard: ${TABLE}.dashboard_id is not null
      LookML Dashboard: ${TABLE}.lookml_dashboard_id is not null
  ...
  - dimension: run_status
    sql_case:
      rendering: ${rendering_started_raw} is not null
      enqueued: ${processing_started_raw} is not null
      scheduled: ${scheduled_raw} is not null
      resting: 1 = 1
  ...
  - dimension_group: processing_finished
    type: time
    timeframes: [raw, time, date, week, month, quarter, year, minute, minute2, minute5, hour, hour_of_day, day_of_week]
  ...
  - measure: count_with_image
    ...
    drill_fields: detail*
    filters:
      should_have_image: Yes

  - measure: count_without_image
    type: count
    drill_fields: detail*
    filters:
      should_have_image: No
