- view: dashboard
  ...
  - dimension: id
    ...
    sql: |
       {% if _dialect._name == 'hypersql' %} CONVERT(${TABLE}.ID, SQL_VARCHAR) {% else %} CAST(${TABLE}.id AS CHAR(256)) {% endif %}
  ...
  - dimension: title
    sql: ${TABLE}.TITLE
    html: |
       <a href="/dashboards/{{dashboard.id._value }}">{{value}}</a>
  ...
  - dimension: link
    sql: ${id}
    html: |
      [<a href="/dashboards/{{ value }}">Dashboard</a>]
